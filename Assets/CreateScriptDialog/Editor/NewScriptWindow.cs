...
public class NewScriptWindow : EditorWindow
{
	...
	class Styles
	{
		public GUIContent m_WarningContent = new GUIContent (string.Empty, EditorGUIUtility.LoadRequired("console.warnicon.sml.png") as Texture2D);
    ...
    if (CanAddComponent())
    {
      //InternalEditorUtility.AddScriptComponentUnchecked (m_GameObjectToAddTo,	AssetDatabase.LoadAssetAtPath (TargetPath (), typeof (MonoScript)) as MonoScript);
      MethodInfo m = typeof(InternalEditorUtility).GetMethod("AddScriptComponentUncheckableUndoable", BindingFlags.Static | BindingFlags.NonPublic);
      m.Invoke(null, new Object[] { m_GameObjectToAddTo, AssetDatabase.LoadAssetAtPath(TargetPath(), typeof(MonoScript)) as MonoScript });
    }
		Close ();
		GUIUtility.ExitGUI ();
	}
