...
public static class ExtensionMethods
{
  ...
  public static Mesh CombineMeshes(this GameObject aGo) 
  {
    ...
    if(meshRenderers != null && meshRenderers.Length > 0) 
    {
      foreach(MeshRenderer meshRenderer in meshRenderers) 
      {
        ...
        if(filter != null && filter.sharedMesh != null) 
        {
          ...
          //Debug.Log("GameObject [ " + filter.gameObject.transform.parent.name + " ] -> current " + filter.sharedMesh.vertexCount + " total " + totalVertexCount); 
        }
      }
    }
    ...
    {
      Debug.LogWarning("There are too many vertices to combine into 1 mesh ("+totalVertexCount+"). The max. limit is 65535");
      return null;
    }
    ...
    if(meshRenderers != null && meshRenderers.Length > 0) 
    {
      ...
      {        
        ...
        if(filter != null && filter.sharedMesh != null && filter.gameObject.activeSelf) 
        {
          ...
          {   
            filter.gameObject.GetComponent<MeshRenderer>().enabled = false;
